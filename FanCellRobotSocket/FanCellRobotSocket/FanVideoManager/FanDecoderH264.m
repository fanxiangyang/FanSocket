...
#import "FanDecoderH264.h"

@implementation FanDecoderH264{
...
-(void)fan_decodeVideoH264:(NSData *)videoData length:(long)length decodedVideoDataBlock:(FanDecodedVideoDataBlock)decodedVideoDataBlock{
    self.decodedVideoDataBlock  = decodedVideoDataBlock;
    
    packetBuffer = (unsigned char *)[videoData bytes];
    packetSize = length;
//    NSLog(@"%@",videoData);
    [self updateFrame];
}
    ...
    dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        *pNalSize = CFSwapInt32HostToBig(nalSize);
        ...
        switch (nalType) {
            case 0x05:
                //                NSLog(@"*********** IDR frame, I帧");
                [self fan_initVideoToolBox]; // 当读入IDR帧的时候初始化VideoToolbox，并开始同步解码
        ...
        if(pixelBuffer) {
            ...
            self.decodedVideoDataBlock(pixelBuffer);
...
- (void)fan_initVideoToolBox {
    ...
    //        NSData *sps=[NSData dataWithBytes:mSPS length:mSPSSize];
    //        NSData *pps=[NSData dataWithBytes:mPPS length:mPPSSize];
    //
        ...
        if(status == kCMBlockBufferNoErr) {
            ...
            if (status == kCMBlockBufferNoErr && sampleBuffer) {
                ...
                if(decodeStatus == kVTInvalidSessionErr) {
                    ...
                    //重新调用一下fan_initVidoBox  锁屏后，重新启动会失效mDecodeSession
