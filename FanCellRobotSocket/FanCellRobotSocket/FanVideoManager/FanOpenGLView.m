...
@interface FanOpenGLView ()
{
    ...
    const GLfloat   *_preferredConversion;//选择转换
    
    BOOL            _isFullScreen;//是否满屏
    
    CGRect layerBound;//解决在子线程中，self.layer  self.bounds  不能访问
    
}

@property GLuint program;

- (void)setupBuffers;//设置需要空间
- (void)cleanUpTextures;//清理图像数据

- (BOOL)loadShaders;//加载着色器
- (BOOL)compileShader:(GLuint *)shader type:(GLenum)type URL:(NSURL *)URL;//完成着色器着色
- (BOOL)linkProgram:(GLuint)prog;
- (BOOL)validateProgram:(GLuint)prog;

@end

@implementation FanOpenGLView
/*
...
- (void)fan_displayPixelBuffer:(CVPixelBufferRef)pixelBuffer  // ---- pixelBuffer:解码后的视频数据 ----
{
    ...
    // 设置四顶点相对于视频的方向和长宽比。
    CGRect vertexSamplingRect = AVMakeRectWithAspectRatioInsideRect(CGSizeMake(_backingWidth, _backingHeight), layerBound);
    
    // 计算标准化的四个坐标来绘制帧。
    CGSize normalizedSamplingSize = CGSizeMake(0.0, 0.0);
    CGSize cropScaleAmount = CGSizeMake(vertexSamplingRect.size.width/layerBound.size.width, vertexSamplingRect.size.height/layerBound.size.height);
...
#pragma mark -  OpenGL ES 2 shader compilation  加载着色器
// 着色器编译
- (BOOL)loadShaders
{
